...
makeSuiteCleanRoom("TargetedCampaignReferenceModule", function () {
  ...
  describe("constructor", () => {
    it("sets storage", async () => {
      expect((await referenceModule.protocolFeeBps()).toNumber()).to.equal(
        CAMPAIGN_FEE_BPS
      );
    });
  });
  ...
  describe("#initializeReferenceModule", () => {
    ...
    context("context: with invalid params", () => {
      ...
      it("reverts when the caller has not approved the transfer of the budget plus fee", async () => {
        ...
        await currencyContract
          .connect(publisher)
          .approve(referenceModule.address, budget);
    ...
    context("context: with valid params", () => {
      ...
      beforeEach(async () => {
        ...
        await currencyContract
          .connect(publisher)
          .approve(referenceModule.address, totalAmount);
      ...
      it("initializes the module, transfers the budget + fee to the contract, and sets protocol fees in storage", async () => {
        ...
        expect(
          (
            await referenceModule.protocolFeesPerCurrency(
      ...
      describe("#getMerkleRootForPublication", () => {
        it("returns the correct merkle root", async () => {
          const res = await referenceModule.getMerkleRootForPublication(
            FIRST_PROFILE_ID,
            FIRST_PUB_ID
          );
    ...
    context("context: with client fees set to non-zero", () => {
      let referenceModuleInitData: any;
      let totalAmount: any;
      let clientFee: any;
      let totalAmountPlusClientFee: any;
      ...
      it("initializes the module, transfers the budget + fee to the contract, and sets client fees in storage", async () => {
        ...
        await currencyContract
          .connect(publisher)
          .approve(referenceModule.address, totalAmountPlusClientFee);
        await expect(
          lensHub.connect(publisher).post({
            profileId: FIRST_PROFILE_ID,
            contentURI: MOCK_URI,
        ...
        expect(
          (
            await referenceModule.getClientFeePerMirrorForPublication(
              FIRST_PROFILE_ID,
              FIRST_PUB_ID
            )
          ).toString()
          ...
          clientFee
            .div(BigNumber.from(DEFAULT_TOTAL_PROFILES.toString()))
            .toString()
        );
      });
    });
  ...
  describe("#processMirror", () => {
    context("context: with a valid merkle tree", () => {
      ...
      it("mirrors the post, distributes from the reward pool, and updates storage", async () => {
        ...
        const budgetRemaining =
          await referenceModule.getBudgetRemainingForPublication(
            FIRST_PROFILE_ID,
            FIRST_PUB_ID
          );
        const expected = parseEther(DEFAULT_BUDGET).sub(
          parseEther(DEFAULT_BUDGET_PER_PROFILE)
        );
        expect(budgetRemaining.toString()).to.equal(expected.toString());

        expect(
          await referenceModule.campaignRewardClaimed(
            FIRST_PROFILE_ID,
            FIRST_PUB_ID,
            FIRST_PROFILE_ID
          )
    ...
    context("context: with client fees set to non-zero...two mirrors", () => {
      ...
      const budgetPerProfile = parseEther("1");
      let protocolFee: any;
      let clientFee: any; // the total fee for this campaign
      ...
      context(
        "context: with a non-whitelisted client address encoded in the module data",
        () => {
          it("only accrues client fees for one mirror and keeps the unclaimed client fees for the protocol", async () => {
            ...
            expect(
              (
                await referenceModule.clientFeesPerCurrency(
                  deployer.address,
                  currencyContract.address
                )
              ).toString()
      ...
      context(
        "context: with a whitelisted client address encoded in the module data",
        () => {
  ...
  describe("#withdrawBudgetForPublication", () => {
    it("reverts when the caller is not the profile owner", async () => {
      await expect(
        referenceModule
          .connect(user)
          .withdrawBudgetForPublication(FIRST_PROFILE_ID, FIRST_PUB_ID)
    ...
    context("context: with an active campaign", async () => {
      ...
      beforeEach(async () => {
        ...
        const protocolFeeRemaining = await referenceModule.getProtocolFee(
          budgetRemaining
        );
        const clientFeeRemaining = await referenceModule.getClientFee(
          budgetRemaining
        );
  ...
  describe("#setProtocolFeeBps", () => {
    ...
    it("updates storage and emits an event", async () => {
      ...
      expect((await referenceModule.protocolFeeBps()).toString()).to.equal(
        value.toString()
      );
  ...
  describe("#setClientFeeBps", () => {
    ...
    it("updates storage and emits an event", async () => {
      ...
      expect((await referenceModule.clientFeeBps()).toString()).to.equal(
        value.toString()
      );
  ...
  describe("#setClientWhitelist", () => {
    ...
    it("updates storage and emits an event", async () => {
      ...
      expect(
        await referenceModule.whitelistedClients(deployer.address)
      ).to.equal(value);
  ...
  describe("#withdrawProtocolFees", () => {
    it("reverts when the caller is not the contract owner", async () => {
      await expect(
        referenceModule
          .connect(user)
          .withdrawProtocolFees(currencyContract.address)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
    ...
    context("context: when there are fees accrued", async () => {
      ...
      it("transfers the fees without affecting the remaining budget, and emits an event", async () => {
        const tx = referenceModule.withdrawProtocolFees(
          currencyContract.address
        );
        const txReceipt = await waitForTx(tx);
  ...
  describe("#withdrawClientFees", () => {
    it("reverts when the caller is not a whitelisted client", async () => {
      await expect(
        referenceModule
          .connect(user)
          .withdrawClientFees(currencyContract.address)
      ).to.be.revertedWith("OnlyWhitelistedClients");
    });

    it("does nothing when there is no balance to withdraw", async () => {
      await referenceModule.setClientWhitelist(deployer.address, true);
      await expect(referenceModule.withdrawClientFees(currencyContract.address))
        .to.not.be.reverted;
